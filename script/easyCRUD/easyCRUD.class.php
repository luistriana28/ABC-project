...
class Crud {
	...
	public function all_join(){
		$col = $this->db->query("SELECT GROUP_CONCAT(COLUMN_NAME) AS 'COLUMNS' FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '". $this->table ."' AND COLUMN_NAME NOT IN ('". $this->fk ."')");
		return $this->db->query("SELECT ". $this->table .".". str_replace(',',','.$this->table.'.',$col[0][COLUMNS]) .", ". $this->table_f .".". $this->value_id_fk ." AS ". $this->fk ." FROM ". $this->table ." LEFT JOIN ". $this->table_f ." ON ". $this->table .".". $this->fk ." = ". $this->table_f .".". $this->value_id_fk ."");
	}
